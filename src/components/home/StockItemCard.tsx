...
export function StockItemCard({ item, onRefill, onRestock, onEdit, onDelete, onHistory, onDateCorrect }: StockItemCardProps) {
  ...
  const isOutOfStock = !isTask && item.stockQuantity === 0;
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-800">
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        {/* Badge: stock quantity (item) or task label */}
        {isTask ? (
          <div className="flex-shrink-0 px-2 py-1 rounded-lg text-xs font-medium bg-indigo-50 text-indigo-600 dark:bg-indigo-950 dark:text-indigo-400">
            作業
          </div>
        ) : (
          <div className={cn(
